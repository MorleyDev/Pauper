...
function renderOrigin({ document, canvas, context, assets }: Target, command: Origin): void {
	const origin = command[1];
	context.translate(origin.x | 0, origin.y | 0);
	renderToCanvas({ document, canvas, context, assets }, command[2]);
	context.translate(-origin.x | 0, -origin.y | 0);
}

function renderRotate({ document, canvas, context, assets }: Target, command: Rotate): void {
...
function renderScale({ document,canvas, context, assets }: Target, command: Scale): void {
...
function getOrCreateCanvasCacheRef(document: Document, key: string, width: number, height: number): CanvasCacheRef {
	return canvasCache[key] || createCanvasCacheRef(document, key, width, height);
}

function renderRenderTarget({ document, canvas, context, assets }: Target, [_, key, dst, frames, size]: RenderTarget): void {
	const width = (size == null ? dst.width : size.x) | 0;
	const height = (size == null ? dst.height : size.y) | 0;

	const targetCanvas = getOrCreateCanvasCacheRef(document, key, width, height);
	if (frames != null && targetCanvas.frames !== frames) {
		renderToCanvas({ document, canvas: targetCanvas.canvas, context: targetCanvas.context, assets }, frames);
	}
	context.drawImage(targetCanvas.canvas, dst.x | 0, dst.y | 0, dst.width | 0, dst.height | 0);
}

function createCanvasCacheRef(document: Document, key: string, width: number, height: number): CanvasCacheRef {
